...
<div id="drawerHamburger" class={`fixed w-[95%] md:max-w-[430px] left-2/4 -translate-x-1/2 transition-all duration-500 max-h-[80vh] h-auto overflow-y-auto ${isOpen ? 'opacity-100 bottom-14' : 'opacity-0 -bottom-full'} ease-in-out z-[119]`}>
    <div class="bg-accent1 h-auto rounded-t-xl pb-20">
        <div class="flex justify-between items-center p-4 border-b">
            <a href="/" class="cursor-pointer" aria-label="Hamilton Place - Home" on:click={closeModal}>
            ...
            <div role="navigation" class="navbar flex flex-col order-3 md:order-1">
                <ul class="list-none px-7 pt-7 pb-3 text-center flex flex-col">
                    ...
                    <li class="mb-1 order-6 md:order-4">
                        <a href="/about" class="text-white hover:text-accent2 transition-all duration-700 ease-in-out font-semibold uppercase leading-loose" aria-label="Navigate to About" on:click={closeModal}>About</a>
                    </li>
                    <li class="mb-1 order-5">
                        <a href="/tenant" class="text-white hover:text-accent2 transition-all duration-700 ease-in-out font-semibold uppercase leading-loose" aria-label="Navigate to a Tenant Page" on:click={closeModal}>Altar'd State</a>
                    </li>
                    <li class="mb-1 order-4 md:order-6">
                        <a href="/deals" class="text-white hover:text-accent2 transition-all duration-700 ease-in-out font-semibold uppercase leading-loose" aria-label="Navigate to the Deals Collection Page" on:click={closeModal}>Deals</a>
                    </li>
                    <li class="mb-1 order-3 md:order-7">
                        <a href="/events" class="text-white hover:text-accent2 transition-all duration-700 ease-in-out font-semibold uppercase leading-loose" aria-label="Navigate to the Events Collection Page" on:click={closeModal}>Events</a>
                    </li>
                    <li class="mb-1 order-2 md:order-8">
                        <a href="/jobs" class="text-white hover:text-accent2 transition-all duration-700 ease-in-out font-semibold uppercase leading-loose" aria-label="Navigate to the Jobs Collection Page" on:click={closeModal}>Jobs</a>
                    </li>
                    <li class="mb-1 order-1 md:order-9">
                        <a href="/news" class="text-white hover:text-accent2 transition-all duration-700 ease-in-out font-semibold uppercase leading-loose" aria-label="Navigate to the News Collection Page" on:click={closeModal}>News</a>
                    </li>
                </ul>
            </div>

            <div id="socials" class="flex flex-row justify-center space-x-4 order-2">
                <a href="#" target="_blank" class="flex p-1 w-8 h-8 leading-8 m-2 text-light hover:text-accent2" aria-label="Link to our Instagram" rel="noopener noreferrer" on:click={closeModal}>
                ...
                <a href="#" target="_blank" class="flex p-1 w-8 h-8 leading-8 m-2 text-light hover:text-accent2" aria-label="Link to our Facebook" rel="noopener noreferrer" on:click={closeModal}>
                    <i class="fab fa-facebook-f text-2xl text-center"></i>
                    <span aria-hidden="true"><i class="fa-solid fa-square-up-right text-white sr-only"></i></span><span class="sr-only">(opens in a new tab)</span>
                </a>
                <a href="#" target="_blank" class="group flex p-1 w-8 h-8 leading-8 m-2" aria-label="Link to our TripAdvisor" rel="noopener noreferrer" on:click={closeModal}>
