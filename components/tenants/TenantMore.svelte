...
<div id="TenantMore" class="my-0 p-0 mx-auto relative" aria-labelledby="moreFromAltardState">
  <h2 id="moreFromAltardState" class="h2-alt px-12 pt-12 text-center">More from <span class="alt">Altar'd State</span><span>.</span></h2>
  <div id="navButtons" class="top-[50%] absolute z-50 w-screen">
    <button class="prev left-1"><i class="far fa-arrow-alt-circle-left fa-2x"></i></button>
    <button class="next right-1"><i class="far fa-arrow-alt-circle-right fa-2x"></i></button>
  </div>
  <div class="product-carousel flex w-full h-auto md:h-80 pb-5 px-0 m-0 overflow-visible">
    {#each columns as column, index (index)}
        <div class={`content h-auto md:h-64 py-6 border shadow-md w-full flex flex-col md:flex-row md:flex-grow-1 ${bgColors[index % bgColors.length]}`}>
            <div class="h-full flex flex-col md:flex-row md:justify-center px-6 md:px-3 md:flex-1">
                <figure class="figParent w-auto md:w-2/5 h-96 md:h-64 rounded flex flex-row md:flex-col flex-shrink-0 shadow-xl">
                    <img src={column.imageUrl} alt={column.title} class="w-full h-full md:w-full md:h-full object-cover object-center rounded" />
                </figure>
                <div class="description w-full md:w-3/5 flex flex-row md:flex-col justify-start px-4 py-2 overflow-hidden">
                    <div class="flex flex-col w-full h-full">
                        <div class="product-title text-lg text-white text-nowrap leading-none font-black truncate">{column.title}</div>
                        <div class="product-store text-white text-sm opacity-75">{column.tenant}</div>
                        <div class="product-sum text-white text-md mt-3 md:mt-5 mb-12 md:mb-0 overflow-hidden" style="-webkit-line-clamp: 2; display: -webkit-box; -webkit-box-orient: vertical;">{column.description}</div>
