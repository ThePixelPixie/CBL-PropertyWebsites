<script>
  ...
  // Define tweened for smooth transition
  const y = tweened(0, { duration: 200, easing: cubicOut }); // Reduced duration for faster response
  ...
  onMount(() => {
    ...
    const handleScroll = () => {
      ...
      } else if (currentScroll - lastScroll > 5) { // Added a threshold to reduce flickering
        isScrollingDown = true;
      } else if (currentScroll - lastScroll < -5) { // Added a threshold to reduce flickering
        isScrollingDown = false;
      }
      lastScroll = currentScroll;
      y.set(isScrollingDown ? -110 : 0); // Translate header up or down
    };
...
<header id="logo-bar" class="fixed h-auto top-0 left-0 pl-0 pr-0 flex flex-col w-full will-change-transform transition-all duration-700 ease-in-out bg-light-80 z-50 shadow-custom backdrop-blur-8" bind:this={header} style="transform: translateY({headerY}%)">
  <AlertBar />
  <div class="flex grow w-full px-2 md:px-4 py-2">
    <div class="flex justify-start items-center w-1/2 md:w-3/4 space-x-4">
      <a href="/" class="cursor-pointer h-full transform duration-500 ease-in-out hover:opacity-75" aria-label="Hamilton Place - Home">
        <svg id="Logo" class="w-11/12 md:w-auto h-full md:max-h-12 fill-current text-dark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 396.51 96.42">
          <g>
    ...
    <div class="flex flex-nowrap w-1/2 md:w-1/4 justify-end">
      <div class="w-0 md:w-[40px] clockbox mt-2 hidden md:block">
        <svg id="clock" class="w-[36.6px] h-[36.6px]" xmlns="http://www.w3.org/2000/svg" width="300" height="200" viewBox="0 0 600 600">
          <g id="face">
            <circle class="circle fill-none stroke-dark" cx="300" cy="300" r="253.9" />
            <path class="hour-marks fill-none stroke-dark" d="M300.5 94V61M506 300.5h32M300.5 506v33M94 300.5H60M411.3 107.8l7.9-13.8M493 190.2l13-7.4M492.1 411.4l16.5 9.5M411 492.3l8.9 15.3M189 492.3l-9.2 15.9M107.7 411L93 419.5M107.5 189.3l-17.1-9.9M188.1 108.2l-9-15.6" />
            <circle class="mid-circle fill-dark" cx="300" cy="300" r="16.2" />
          </g>
          <g id="hour" class="fill-none transition-transform duration-500 ease-in-out origin-center" bind:this={hourGroup}>
            <path class="hour-arm fill-none stroke-dark" d="M300.5 298V142" />
            <circle class="sizing-box" cx="300" cy="300" r="253.9" />
          </g>
          <g id="minute" class="transition-transform duration-500 ease-in-out origin-center" bind:this={minuteGroup}>
            <path class="minute-arm fill-none stroke-dark" d="M300.5 298V67" />
            <circle class="sizing-box fill-none" cx="300" cy="300" r="253.9" />
          </g>
        </svg>
      </div>
      <div class="w-auto flex flex-col justify-center pl-2 md:pl-4 mt-1 text-left text-clamp-xs">
        <span class="block font-bold">Open Now</span>
        10:00am - 8:00pm
      </div>
    </div>
  </div>
