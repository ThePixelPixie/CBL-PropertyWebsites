...
<section id="DealsIsotopeGrid" class="grid-container w-full max-w-7xl mx-auto px-4 pb-24 overflow-visible relative">
  ...
  <div class="filter-group relative w-full overflow-visible space-y-4 md:space-y-8">
    ...
    {#each items as item}
    <a role="button" class={`grid-item border w-full md:w-[48%] lg:w-[31%] my-2 md:my-4 mx-auto md:mx-[1%] md:float-left gap-3 flex flex-row items-center justify-stretch relative md:block visible cursor-pointer overflow-hidden focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${getBorderFocusClass(item.type)} ${getBorderClass(item.type)} ${getBackgroundClass(item.type)} text-white border-solid rounded shadow-lg hover:shadow-none hover:scale-99 relative`} data-type={item.type} data-date={item.startDate} data-search-terms={item.searchTerms.toLowerCase()} data-sub-filter-terms={item.subFilterTerms.toLowerCase()} on:click={() => window.location.href = item.url} tabindex="0"  on:keydown={(e) => e.key === 'Enter' && (window.location.href = item.url)} aria-label={`Read more about the ${item.storeName} deal`}>
